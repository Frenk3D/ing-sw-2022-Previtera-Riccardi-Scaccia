...
//if a client has to send/write something is an ask (from the controller) or it's a show method (like the sync message or else automatic)
...
public interface View {
     ...
     */
    void askPlayerInfo(String playerName);

    void askServerConfig(int serverIp,int serverPort);

    void askNewOrJoinGame();

    void showAvailableLobbies(List<Lobby> lobbiesList);

    void askRequestedLobby(String lobbyName);
    ...
    void showDashboards(Dashboard dashboard,int playerId);

    void showCharacterTable(List<Character> charactersList, AtomicInteger tableMoney, Map<Player,AtomicInteger> numOfMoneyMap);

    void askAssistantsList();

    void showAssistantsList(List<Assistant> assistantsList,int playerId);

    void askStudentToMoveIsland(List<Island> entranceStudentsList,List<Island> islandList);
