...
public class ClientInputVerifier {
    ...
    public static boolean verifyAssistant(ClientGameModel clientGameModel, int selectedAssistant) {
        ...
        //my assistant was thrown and
        if (!myAssistants.contains(selectedAssistant)) {
            return false;
        } else return !thrownAssistants.contains(selectedAssistant) || thrownAssistants.containsAll(myAssistants);
    }
    ...
    public static boolean verifyStudentDashboard(ClientGameModel clientGameModel, int selectedStudent) {
        if (selectedStudent < 0) {
            ...
            return clientGameModel.findPlayerById(clientGameModel.getMyPlayerId()).getDashboard().getEntranceList().size() > selectedStudent;
    }
    ...
    public static boolean verifyStudentIsland(ClientGameModel clientGameModel, int selectedStudent, int selectedIsland) {
        if (selectedStudent < 0 || selectedIsland < 0) {
            ...
            return false;
        } else return clientGameModel.getIslandList().size() > selectedIsland;
    }
    ...
    public static boolean verifyMotherNaturePos(ClientGameModel clientGameModel, int selectedPos, int usedCharacter) {
        ...
        if (usedCharacter != 4) {
            if (selectedPos < 0 || selectedPos == clientGameModel.getMotherNaturePos()) {
                ...
                return (selectedPos - clientGameModel.getMotherNaturePos() >= 0) || ((clientGameModel.getIslandList().size() - clientGameModel.getMotherNaturePos() + selectedPos) <= assistant.getMotherNaturePosShift());
        } else {
            if (selectedPos < 0 || selectedPos == clientGameModel.getMotherNaturePos()) {
                return false;
            } else if (clientGameModel.getIslandList().size() <= selectedPos) {
                ...
                return (selectedPos - clientGameModel.getMotherNaturePos() >= 0) || ((clientGameModel.getIslandList().size() - clientGameModel.getMotherNaturePos() + selectedPos) <= assistant.getMotherNaturePosShift() + 2);
    ...
    public static boolean verifyCloud(ClientGameModel clientGameModel, int selectedCloud) {
        if (selectedCloud < 0) {
            ...
            return false;
        } else return !clientGameModel.getCloudList().get(selectedCloud).getStudentsList().isEmpty();
    }
    ...
    public static boolean verifyCharacter(ClientGameModel clientGameModel, int selectedCharacter) {
        ...
        if (reducedCharacter == null) {
            ...
            return clientGameModel.findPlayerById(clientGameModel.getMyPlayerId()).getNumOfMoney() >= cost;
        }
    }
